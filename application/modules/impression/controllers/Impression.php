...
class Impression extends Del {
	...
	public function brand($brand_id='')
	{
		...
		if(isset($_GET['srchterm']) ||isset($_GET['cat']) || isset($_GET['sil'])|| isset($_GET['neck']) || isset($_GET['waist']))
		{
			...
			$config['total_rows'] =count($intersect);
			$this->pagination->initialize($config); 
			if(isset($intersect) && count($intersect))
			{
				if($brand_id)
				{
					$intersect1=array_slice($intersect, $this->uri->segment(4), $config['per_page']);
	...
	public function testmail(){
	    $this->load->library('mailgun');
			...
			$msg.="\n\nWith love, \nImpression Bridal";
	    $this->mailgun
	        ->to('msakore@gmail.com',"Mahesh Sakore")
	        ->from('info@impressionrbidalstore.com','Impression Bridal')
	        ->subject('Welcome, Impression Bridal')
	        ->message($msg);
	       // ->attachments(array('FILENAME'))
	        ...
	        if($this->mailgun->send())
			{
				...
				$data['error'] = $this->mailgun->print_debugger(array('headers'));
				$this->mprint($data);
			}
/*
			...
			$this->email->to('msakore@gmail.com');//$input['first_name']
